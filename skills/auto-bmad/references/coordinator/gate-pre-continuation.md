...
**Harness note:** Checks 2 and 3 require session state data. On Claude Code, this is available via the session-state hook installed by `/auto-bmad setup` (Step 3). On other harnesses this data may not be available — each check gracefully skips if its fields are absent.
...
**If the file does not exist** — print `"⚠️ Pre-Continuation: session state unavailable (.claude/auto-bmad-session-state.json missing — check that the session-state hook is installed via /auto-bmad setup Step 3). Skipping rate limit checks."` and proceed.
...
1. Convert reset epoch to human-readable time:
   ...
   date -d @{resets_at}
   ```
2. Print: `"⏸ 5-hour usage limit at {usage}% — auto-pausing until reset at {reset_time}. BMAD will resume automatically."`
3. **If `TIMER_SUPPORT=true`:** compute a cron expression 10 minutes after the reset epoch (to avoid a false positive if the reset lands slightly late) and schedule a resume:
   ...
   - `recurring`: `false`
   - `prompt`: `"BMAD_RATE_LIMIT_TIMER_FIRED (five_hour) — The 5-hour rate limit window has reset. Re-check five_hour.used_percentage; if now below API_FIVE_HOUR_THRESHOLD, continue with Pre-Continuation Check 3 (seven-day). If still too high, schedule another pause until the next reset time."`

   Save the job ID. Do not ask the user for input — resume automatically when `BMAD_RATE_LIMIT_TIMER_FIRED` arrives.
...
1. Convert reset epoch to human-readable time:
   ...
   date -d @{resets_at}
   ```
2. Print: `"⏸ 7-day usage limit at {usage}% — auto-pausing until reset at {reset_time}. BMAD will resume automatically."`
3. **If `TIMER_SUPPORT=true`:** compute a cron expression 10 minutes after the reset epoch (to avoid a false positive if the reset lands slightly late) and schedule a resume:
   ```bash
   # macOS
   date -r $(( {resets_at} + 600 )) '+%M %H %d %m *'
   ...
   - `cron`: expression from above
   - `recurring`: `false`
   - `prompt`: `"BMAD_RATE_LIMIT_TIMER_FIRED (seven_day) — The 7-day rate limit window has reset. Re-check seven_day.used_percentage; if now below API_SEVEN_DAY_THRESHOLD, continue with Phase 0. If still too high, schedule another pause until the next reset time."`
